...
function getStateOfChat() {
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/android/index.php/member/chat/"+userid+"/"+localStorage.userid+"//"+chatid+"/?callback=?",
...
function updateChatx() {
	...
	if(!instanse){
		...
		$.ajax({
			...
			crossDomain: true,
			cache: false,
			url: "https://www.dfunstation.com/api4/android/index.php/member/chat/"+userid+"/"+localStorage.userid+"/"+localStorage.lastchatid+"/"+chatid+"/?callback=?",
			...
			success: function(data) {
				...
				instanse = false;
				state = data.state;
				if(data.finish == 1 && actionx !="finish") {
