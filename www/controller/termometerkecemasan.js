$(document).ready(function () {
  ...
  var urlx = base_url + "index.php/konsultasi/gambaran/?callback=?";
  $.ajax({
    type: "GET",
    url: urlx,
    crossDomain: true,
  ...
  $("#signup").click(function () {
    var urls =
      "https://www.dfunstation.com/api4/android/index.php/konsultasi/daftar_termometer/" + localStorage.userid + "/?callback=?";
    ...
    var gambaran = $("#gambaran").val();

    if ($.trim(telepon).length < 1) {
      swal("Telepon masih kosong atau salah pengetikan");
    }

    if ($.trim(gambaran).length < 1) {
      swal("Kondisi saat ini tolong diisi");
    }
