$(document).ready(function () {
  ...
  var urlx = base_url + "index.php/konsultasi/depresi_soal/?callback=?";
  $.ajax({
    type: "GET",
    url: urlx,
    crossDomain: true,
    ...
    success: function (data) {
      if (data["status"] == "OK") {
        ...
        for (x in dl) {
          ...
          datalist += "<br>";
          var jawaban = dl[x]["jawaban"];
          var hasil;
          for (hasil in jawaban) {
            datalist += '<div class="pemilihan">';
            datalist += '<input type="radio" id="pilihan' + jawaban[hasil]["soalid"] + 'dari' + jawaban[hasil]["jawabanid"] + '"  class="pilihan' + jawaban[hasil]["soalid"] + '"  name="pilihan' + jawaban[hasil]["soalid"] + '" value="' + jawaban[hasil]["nilai"] + '" onclick="pilih(' + jawaban[hasil]["soalid"] + ')"  >';
            datalist += '<label for="pilihan' + jawaban[hasil]["soalid"] + 'dari' + jawaban[hasil]["jawabanid"] + '" >' + jawaban[hasil]["jawaban"] + '</label>';
            datalist += "</div>";
          }
          datalist += "</div>";
          datalist += "</div>";
  ...
  $("#hasil").click(function () {
    var urls = base_url + "index.php/konsultasi/depresi_kirim/" + localStorage.userid + "/?callback=?";
    ...
    var hasil18 = $("input:checked" + ".pilihan22").val();
    var hasil19 = $("input:checked" + ".pilihan19").val();
    var hasil20 = $("input:checked" + ".pilihan20").val();
    var hasil21 = $("input:checked" + ".pilihan21").val();


    if ($.trim(hasil21).length < 1) {
      swal("Tolong isi dulu pertanyaan ke 21");
    }
    ...
    var dataString = "hasil1=" + hasil1 + "&hasil2=" + hasil2 + "&hasil3=" + hasil3 + "&hasil4=" + hasil4 + "&hasil5=" + hasil5 + "&hasil6=" + hasil6 + "&hasil7=" + hasil7 + "&hasil8=" + hasil8 + "&hasil9=" + hasil9 + "&hasil10=" + hasil10 + "&hasil11=" + hasil11 + "&hasil12=" + hasil12 + "&hasil13=" + hasil13 + "&hasil14=" + hasil14 + "&hasil15=" + hasil15 + "&hasil16=" + hasil16 + "&hasil17=" + hasil17 + "&hasil18=" + hasil18 + "&hasil19=" + hasil19 + "&hasil20=" + hasil20 + "&hasil21=" + hasil21;

    if (
      $.trim(hasil21).length > 0
    ) {
      $.ajax({
        type: "POST",
        url: urls,
